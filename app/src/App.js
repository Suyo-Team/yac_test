...
const onAuthStateChange = (callback) => {
  return app.auth().onAuthStateChanged(async (user) => {
    if(user){
      let querySnapshot = await app.firestore().collection('users').where('email', '==', user.email).get()
      let nickname = ''
      let url = ''
  ...
  if(isAuth.update){
    return (
      <div style={{width: '100%', height: '100vh', backgroundColor: 'white', display: 'flex', placeContent:'center', placeItems: 'center'}}>
         <WhisperSpinner size={150} color="white" loading={true} /> 
      </div>
    );
  }
  return (
    <div className="App">
      <Router>
        <Switch>
          ...
          <PublicRoute  authentification={isAuth.loggedIn} restricted={true} component={LoginContainer} path="/" exact />
          <PublicRoute  authentification={isAuth.loggedIn} restricted={true} component={LoginContainer} path="/login" exact />
          <PublicRoute  authentification={isAuth.loggedIn} restricted={true} component={RegisterContainer} path="/register" exact />
        </Switch>
      </Router>
    </div>
  );
